...
void findWall() {
    ...
    while (millis() - scanStartTime < (turnDuration * 2)) {
    ...
    Serial.println("Turning to min distance point");

    if (minDistance < 100.0) {
        long timeToTurnBack = (turnDuration * 2) - minDistanceTime;
        setMotorSpeeds(turnLeft);
        delay(timeToTurnBack);
        setMotorSpeeds({0, 0, 0, 0});
        Serial.println("Minimum Distance Point Reached");
        isWallFound = 1;
    } 
    else {
        isWallFound = 0;
